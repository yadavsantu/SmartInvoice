...
export default {
  ...
  methods: {
    async verifyEmail() {
      try {
        const response = await axios.post('smart-invoice-topaz.vercel.app/api/v1/verifyEmail', { email: this.email });
    ...
    Swal.fire({
      ...
      try {
        const response = await axios.post('smart-invoice-topaz.vercel.app/api/v1/ResetPassword', {
          otp: this.otp,
