...
export default {
  ...
  methods: {
    ...
    async verifyEmail() {
      try {
        const response = await axios.post("http://localhost:8080/api/v1/ResetPassword", { email: this.email });
        if (response.status === 200) {
          console.log("Email found");
          this.message = "Recovery email sent successfully.";
        }
      } catch (error) {
        console.error(error);
        this.message = "Failed to send recovery email. Please try again.";
      }
    },
    toggleNewPassword() {
